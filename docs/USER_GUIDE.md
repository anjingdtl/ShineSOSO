...
在 **联网发现索引器** 中输入关键词后，程序会优先查询 Prowlarr 的结构化索引器目录（名称、语言、协议、公开/私有类型），并用公开网页搜索补充候选。中文的“电影、电视剧、动漫、音乐、图书”等关键词会自动扩展为对应的英文分类词。它不会直接把网页加入搜索：只有候选 URL 通过 Torznab 能力探测和连接测试后，才会保存为索引器。
