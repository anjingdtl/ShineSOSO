...
func TestTextMatchScoreExact(t *testing.T) {
    if got := textMatchScore("ubuntu", "Ubuntu 22.04 LTS"); got != 1.0 {
        t.Errorf("substring match should be 1.0, got %v", got)
    }
}

func TestTextMatchScorePartial(t *testing.T) {
    if got := textMatchScore("ubuntu lts", "Ubuntu 22.04"); got != 0.5 {
        t.Errorf("1 of 2 tokens matched, want 0.5, got %v", got)
    }
}
